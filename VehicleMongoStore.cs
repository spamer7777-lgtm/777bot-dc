...
public class VehicleMongoStore
{
    ...
    public async Task<VehicleCard?> GetVehicleAsync(int vuid)
    {
        ...
        // Minimal mapping; keep it tolerant:
        var card = new VehicleCard
        {
            ...
            ModelId = doc.Contains("modelId") ? (int?)doc["modelId"].AsInt32 : null,
            BodykitMainName = doc.Contains("bodykitMain") ? doc["bodykitMain"].AsString : null,
            BodykitAeroName = doc.Contains("bodykitAero") ? doc["bodykitAero"].AsString : null,
            LightsColorRaw = doc.Contains("lightsColor") ? doc["lightsColor"].AsString : null,
            DashboardColorRaw = doc.Contains("dashColor") ? doc["dashColor"].AsString : null,
        };
        ...
        if (doc.Contains("visual") && doc["visual"].IsBsonArray)
        {
            foreach (var v in doc["visual"].AsBsonArray)
            {
                ...
                card.VisualTuning.Add(new VisualItem
                {
                    ...
                    Id = d.Contains("id") && d["id"].IsInt32 ? d["id"].AsInt32 : null
                });
            }
        }
    ...
    public async Task UpsertVehicleAsync(VehicleCard card)
    {
        ...
        if (card.ModelId.HasValue) doc["modelId"] = card.ModelId.Value;
        ...
        doc["visual"] = new BsonArray(card.VisualTuning.Select(v =>
        {
            ...
            if (v.Id.HasValue) vd["id"] = v.Id.Value;
            return vd;
        }));
